...
#define MAX_ITEMS			29 // Don't change
...
ConVar g_hCvarAllow, g_hCvarMPGameMode, g_hCvarModes, g_hCvarModesOff, g_hCvarModesTog, g_hCvarBallNum, g_hCvarBallCol, g_hCvarBallRad, g_hCvarCGift, g_hCvarCTree, g_hCvarEffects, g_hCvarGifts, g_hCvarGlow, g_hCvarHealth, g_hCvarItems, g_hCvarRainbow, g_hCvarRate, g_hCvarSpeed;
float g_fCvarBallRad, g_fCvarSpeed;
int g_iCvarBallNum, g_iCvarBallCol, g_iCvarEffects, g_iCvarGifts, g_iCvarHealth, g_iCvarGlow, g_iCvarRainbow, g_iCvarRate, g_iTotalChance, g_iPlayerSpawn, g_iRoundStart, g_iRoundNumber, g_iSpawnCount, g_iTreesCount, g_iSpawns[MAX_SPAWNS][4], g_iChances[MAX_ITEMS];
...
public void OnPluginStart()
{
	...
	// CVARS
	g_hCvarAllow =		CreateConVar(	"l4d_tree_allow",			"1",			"0=Plugin off, 1=Plugin on.", CVAR_FLAGS );
	g_hCvarModes =		CreateConVar(	"l4d_tree_modes",			"",				"Turn on the plugin in these game modes, separate by commas (no spaces). (Empty = all).", CVAR_FLAGS );
	g_hCvarModesOff =	CreateConVar(	"l4d_tree_modes_off",		"",				"Turn off the plugin in these game modes, separate by commas (no spaces). (Empty = none).", CVAR_FLAGS );
	g_hCvarModesTog =	CreateConVar(	"l4d_tree_modes_tog",		"0",			"Turn on the plugin in these game modes. 0=All, 1=Coop, 2=Survival, 4=Versus, 8=Scavenge. Add numbers together.", CVAR_FLAGS );
	...
	g_hCvarSpeed =		CreateConVar(	"l4d_tree_speed",			"0.2",			"How often to update the colors per second.", CVAR_FLAGS, true, 0.1 );
	...
	// COMMANDS
	RegAdminCmd("sm_tree_spawn",	CmdSpawnerTemp,		ADMFLAG_ROOT, 	"Spawns a temporary Gift/Tree at your crosshair. Usage: sm_tree_spawn [type: 0=Gift. 1=Tree.]");
	RegAdminCmd("sm_tree_save",		CmdSpawnerSave,		ADMFLAG_ROOT, 	"Spawns a Gift/Tree at your crosshair and saves to config. Usage: sm_tree_save [type: 0=Gift. 1=Tree.]");
	RegAdminCmd("sm_tree_del",		CmdSpawnerDel,		ADMFLAG_ROOT, 	"Removes the Gift/Tree you are pointing at and deletes from the config if saved.");
	RegAdminCmd("sm_tree_clear",	CmdSpawnerClear,	ADMFLAG_ROOT, 	"Removes all Gifts/Trees spawned by this plugin from the current map.");
	RegAdminCmd("sm_tree_wipe",		CmdSpawnerWipe,		ADMFLAG_ROOT, 	"Removes all Gifts/Trees from the current map and deletes them from the config.");
	RegAdminCmd("sm_tree_reload",	CmdSpawnerReload,	ADMFLAG_ROOT, 	"Resets the plugin and reloads the data config and saved spawns.");
	if( g_bLeft4Dead2 )
		RegAdminCmd("sm_tree_glow",	CmdSpawnerGlow,		ADMFLAG_ROOT, 	"Toggle to enable glow on all Gifts/Trees to see where they are placed.");
	RegAdminCmd("sm_tree_list",		CmdSpawnerList,		ADMFLAG_ROOT, 	"Display a list Gift/Tree positions and the total number of.");
	RegAdminCmd("sm_tree_tele",		CmdSpawnerTele,		ADMFLAG_ROOT, 	"Teleport to a Gift/Tree (Usage: sm_tree_tele <index: 1 to MAX_SPAWNS (32)>).");
	RegAdminCmd("sm_tree_ang",		CmdSpawnerAng,		ADMFLAG_ROOT, 	"Displays a menu to adjust the Gift/Tree angles your crosshair is over.");
	RegAdminCmd("sm_tree_pos",		CmdSpawnerPos,		ADMFLAG_ROOT, 	"Displays a menu to adjust the Gift/Tree origin your crosshair is over.");
...
public Action CmdSpawnerTemp(int client, int args)
{
	...
	{
		PrintToChat(client, "%sError: Cannot add anymore Trees or Gifts. Used: (\x05%d/%d\x01).", CHAT_TAG, g_iSpawnCount, MAX_SPAWNS);
		return Plugin_Handled;
	}
	...
	{
		PrintToChat(client, "%sError: Cannot add anymore Trees. Used: (\x05%d/%d\x01).", CHAT_TAG, g_iTreesCount, MAX_TREES);
		return Plugin_Handled;
	}
	...
	{
		PrintToChat(client, "%sCannot place Tree, please try again.", CHAT_TAG);
		return Plugin_Handled;
	}
...
public Action CmdSpawnerSave(int client, int args)
{
	...
	if( !hFile.ImportFromFile(sPath) )
	{
		PrintToChat(client, "%sError: Cannot read the Tree config, assuming empty file. (\x05%s\x01).", CHAT_TAG, sPath);
	}
	...
	{
		PrintToChat(client, "%sError: Failed to add map to Tree spawn config.", CHAT_TAG);
		delete hFile;
		return Plugin_Handled;
	}
	...
	if( iCount >= MAX_SPAWNS )
	{
		PrintToChat(client, "%sError: Cannot add anymore Trees. Used: (\x05%d/%d\x01).", CHAT_TAG, iCount, MAX_SPAWNS);
		delete hFile;
		return Plugin_Handled;
	}
	...
	if( hFile.JumpToKey(sTemp, true) )
	{
		...
		PrintToChat(client, "%s(\x05%d/%d\x01) - Failed to save Tree.", CHAT_TAG, iCount, MAX_SPAWNS);
...
public Action CmdSpawnerDel(int client, int args)
{
	...
	if( !FileExists(sPath) )
	{
		PrintToChat(client, "%sError: Cannot find the Christmas Tree config (\x05%s\x01).", CHAT_TAG, CONFIG_SPAWNS);
		return Plugin_Handled;
	}
	...
	if( !hFile.ImportFromFile(sPath) )
	{
		PrintToChat(client, "%sError: Cannot load the Christmas Tree config (\x05%s\x01).", CHAT_TAG, sPath);
		delete hFile;
		return Plugin_Handled;
	}
	...
	if( !hFile.JumpToKey(sMap) )
	{
		PrintToChat(client, "%sError: Current map not in the Christmas Tree config.", CHAT_TAG);
		delete hFile;
		return Plugin_Handled;
	}
	...
	if( bMove )
	{
		...
		PrintToChat(client, "%s(\x05%d/%d\x01) - Tree removed from config.", CHAT_TAG, iCount, MAX_SPAWNS);
	}
	else
		PrintToChat(client, "%s(\x05%d/%d\x01) - Failed to remove Tree from config.", CHAT_TAG, iCount, MAX_SPAWNS);
...
public Action CmdSpawnerClear(int client, int args)
{
	...
	PrintToChat(client, "%s(0/%d) - All Trees and Gifts removed from the map.", CHAT_TAG, MAX_SPAWNS);
	return Plugin_Handled;
}
...
public Action CmdSpawnerWipe(int client, int args)
{
	...
	if( !FileExists(sPath) )
	{
		PrintToChat(client, "%sError: Cannot find the Tree config (\x05%s\x01).", CHAT_TAG, sPath);
		return Plugin_Handled;
	}
	...
	if( !hFile.ImportFromFile(sPath) )
	{
		PrintToChat(client, "%sError: Cannot load the Tree config (\x05%s\x01).", CHAT_TAG, sPath);
		delete hFile;
		return Plugin_Handled;
	}
	...
	{
		PrintToChat(client, "%sError: Current map not in the Tree config.", CHAT_TAG);
		delete hFile;
		return Plugin_Handled;
	}
	...
	PrintToChat(client, "%s(0/%d) - All Trees and Gifts removed from config, add with \x05sm_tree_save\x01.", CHAT_TAG, MAX_SPAWNS);
	return Plugin_Handled;
}
