...
public void OnPluginStart() {
	...
	**/
	...
	g_cvEmotesSounds = CreateConVar("sm_emotes_sounds", "0", "Enable/Disable sounds for emotes.", CVAR_FLAGS);
	...
	g_cvTeleportBack = CreateConVar("sm_emotes_teleportonend", "1", "Teleport back to the exact position when he started to dance. (Some maps need this for teleport triggers)", CVAR_FLAGS);
...
public Action Command_Menu(int client, int args)
{
	...
	{
		CPrintToChat(client, "%T", "SURVIVOR_FIRST",client);
		return Plugin_Handled;
	}
	...
	else CPrintToChat(client, "%T", "NO_DANCES_ACCESS_FLAG",client);	
...
Action CreateEmote(int client, const char[] anim1, const char[] anim2, const char[] soundName, bool isLooped) {
	...
	if (!IsPlayerAlive(client)) {
		CPrintToChat(client, "%T", "MUST_BE_ALIVE",client);
		return Plugin_Handled;
	}

	if (! (GetEntityFlags(client) & FL_ONGROUND)) {
		CPrintToChat(client, "%T", "STAY_ON_GROUND",client);
		return Plugin_Handled;
	}

	if ( GetEntProp(client, Prop_Send, "m_isIncapacitated", 1) || GetEntProp(client, Prop_Send, "m_isHangingFromLedge", 1) ) {
		CPrintToChat(client, "%T", "CANNOT_USE_NOW",client);
		return Plugin_Handled;
	}

	if (CooldownTimers[client]) {
		CPrintToChat(client, "%T", "COOLDOWN_EMOTES",client);
		return Plugin_Handled;
	}

	if (StrEqual(anim1, "")) {
		CPrintToChat(client, "%T", "AMIN_1_INVALID",client);
		return Plugin_Handled;
	}
...
Action Menu_Dance(int client)
{
	...
	AddTranslatedMenuItem(menu, "", "NOTIFY", client);
...
Action EmotesMenu(int client)
{
	...
	if (!CheckAdminFlags(client, ReadFlagString(sBuffer)))
	{
		CPrintToChat(client, "%T", "NO_EMOTES_ACCESS_FLAG",client);
		return Plugin_Handled;
	}
...
Action DancesMenu(int client)
{
	...
	if (!CheckAdminFlags(client, ReadFlagString(sBuffer)))
	{
		CPrintToChat(client, "%T", "NO_DANCES_ACCESS_FLAG",client);
		return Plugin_Handled;
	}
...
Action RandomEmote(int i)
{
					...
					{
						CPrintToChat(i, "%T", "NO_EMOTES_ACCESS_FLAG",i);
						return;
					}
...
Action RandomDance(int i)
{
					...
					if (!CheckAdminFlags(i, ReadFlagString(sBuffer)))
					{
						CPrintToChat(i, "%T", "NO_DANCES_ACCESS_FLAG",i);
						return;
					}
...
Action Command_Admin_Emotes(int client, int args)
{
	if (args < 1)
	{
		CPrintToChat(client, "[SM] Usage: sm_setemotes <#userid|name> [Emote ID]",client);
		return Plugin_Handled;
	}
	...
	if (args > 1)
	{
		...
		{
			CPrintToChat(client, "%T", "INVALID_EMOTE_ID",client);
			return Plugin_Handled;
		}
	}
...
int MenuHandler_EmotePlayers(Menu menu, MenuAction action, int param1, int param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		{
			CPrintToChat(param1, "[SM] %T", "Player no longer available",param1);
		}
		else if (!CanUserTarget(param1, target))
		{
			CPrintToChat(param1, "[SM] %T", "Unable to target",param1);
		}
		else
		{
