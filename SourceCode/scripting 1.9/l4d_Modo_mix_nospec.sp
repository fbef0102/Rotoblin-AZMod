...
1.3 (28-01-2019)
	- no spec captains and no spec chosen members
...
public Plugin myinfo = 
{
	...
	description = "Modo de juego Mix (No Spec)",
...
public Action Command_Captainvote(int client, int args)
{
	...
	{
		CPrintToChat(client, "{default}[{olive}Mix{default}] A mix is already in process.");
		return Plugin_Handled;
	}
...
void DisplayVoteMenuCaptainSurvivor()
{
	if (g_CvarMixStatus.IntValue)
	{
		...
		SurvivorCaptainMenu.SetTitle("Choose Survivor Captain(no spec):");
		int players;
		...
		{
			if (IsClientInGame(i)&&!IsFakeClient(i) && GetClientTeam(i)!= TEAM_SPECTATOR) // no spec
			{
				Format(name, 32, "%N", i);
...
void DisplayVoteMenuPlayerSelect()
{
	if (g_CvarMixStatus.IntValue)
	{
		...
		PlayerSelectMenu.SetTitle("Choose wisely...(no spec)");
		char name[32];
		char number[12];
		for(int i = 1; i <= MaxClients; i++) 
		{
			if ( IsValidClient(i) && !IsFakeClient(i) && !g_bHasBeenChosen[i] && i != g_iSurvivorCaptain && i != g_iInfectedCaptain && GetClientTeam(i)!= TEAM_SPECTATOR)
			{
				Format(name, 32, "%N", i);
				Format(number, 10, "%i", i);
...
public int Handler_PlayerSelectionCallback(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		case 4:
		{
			...
			if (IsClientInGame(target) && !IsFakeClient(target))
			{
				if(GetClientTeam(target) == TEAM_SPECTATOR)
				{
					CPrintToChat(param1,"{default}[{olive}Mix{default}] This player is spectator, aleh says no spec pls, choose again!");
				}
				else
				{
...
void DisplayVoteMenuCaptainInfected()
{
	...
	InfectedCaptainMenu.SetTitle("Choose Infected Captain(no spec):");
	int players;
	g_bHasOneVoted = false;
	char name[32];
	char number[12];
	for(int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && !IsFakeClient(i) && g_iSurvivorCaptain != i && GetClientTeam(i)!= TEAM_SPECTATOR) //no spec
		{
			Format(name, 32, "%N", i);
			Format(number, 10, "%i", i);
