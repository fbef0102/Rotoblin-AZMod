...
public Action:_2V2_SlayInfectedBot(Handle:timer, any:client)
{
	...
	if (IsPlayerAlive(client))//When a spawned Infected Player disconnects, changes team or becomes Tank the SI will instantly get killed unless it’s a Boomer or has someone capped.
	{
		...
		if(zombieclass == 2||zombieclass == 5) //if is boomer or Tank
			return Plugin_Stop;
