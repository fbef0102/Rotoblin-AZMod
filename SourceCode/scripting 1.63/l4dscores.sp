...
#define SCORE_VERSION_REQUIRED_LEFT4DOWNTOWN "0.4.5"
...
PrepareAllSDKCalls()
{
	gConf = LoadGameConfigFile("left4downtown.l4d");
	if(gConf == INVALID_HANDLE)
	{
		LogError("Could not load gamedata/left4downtown.l4d.txt");
		DebugPrintToAll("Could not load gamedata/left4downtown.l4d.txt");
	}
...
{
	if(IsNewMission()||Is_First_Stage()) //only new map
	{
		new scoreA, scoreB;
		
		scoreA = L4DDirect_GetVSCampaignScore(0);
		scoreB = L4DDirect_GetVSCampaignScore(1);
...
public Action:Command_SetCampaignScores(client, args)
{
	...
	*/
	...
	//SDKCall(fSetCampaignScores, score1, score2);//這個只能set score / score
	//L4DDirect_SetVSCampaignScore(team, score2); //設置一個隊伍前面關卡的全部分數
...
public Action:Command_GetTeamScore(client, args)
{
	...
	* sm_getscore 6 0 - health bonus? (completition % in the sacrifice final)
	* 
	* --------------------------------
	* 
...
{
	new maxbonus = GetTeamMaxHumans(L4D_TEAM_SURVIVORS) * 20;
	return (float(Pills) / maxbonus ) * 100;
}

CheckSurvivorProgress()
{
	new P = SDKCall(fGetTeamScore, 6, 0);
