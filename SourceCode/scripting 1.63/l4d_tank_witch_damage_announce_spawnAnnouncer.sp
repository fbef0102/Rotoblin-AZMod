	...
	if (iSurvivors == 1 && !bLoose){
		...
		if (bTankBoss){
			...
			for (new i; i < iSurvivors; i++){
				...
				if (g_iCvarPrivateFlags & (1 << _:TANK)){
					...
					if (bInGame)
						CPrintToChat(client, "{olive}[TS] %T","l4d_tank_witch_damage_announce_spawnAnnouncer5",client, g_iTotalDamage[iIndex][bTankBoss], iClient[i][DMG], RoundToNearest(FloatMul(FloatDiv(float(iClient[i][DMG]), fTotalDamage), 100.0)));
				}
				else{ // public
			...
			if (!(g_iCvarPrivateFlags & (1 << _:TANK))&&g_TankOtherDamage){
				CPrintToChatAll(" {olive}%d{default} [{green}%.0f%%{default}] - {lightgreen}%t",g_TankOtherDamage,FloatMul(FloatDiv(float(g_TankOtherDamage), fTotalDamage), 100.0),"Other damage");
				g_TankOtherDamage = 0;
			}
		}
		else 
		{
			for (new i; i < iSurvivors; i++){
				...
				{
					CPrintToChat(iClient[i][INDEX], "{olive}[TS] %T","l4d_tank_witch_damage_announce_spawnAnnouncer6", iClient[i][INDEX],g_iTotalDamage[iIndex][bTankBoss], i + 1, iClient[i][DMG], RoundToNearest(FloatMul(FloatDiv(float(iClient[i][DMG]), fTotalDamage), 100.0)));
				}
				else
				{
