...
public EventReviveSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(GetEventBool(event, "lastlife"))
	{
		...
		{
			if(bandw_type == 1) PrintHintTextToAll("%t","Player is black and white.", targetName, charName);
			else PrintToChatAll("%t","Player is black and white.", targetName, charName);
		}
		//print to infected
		else if(bandw_notice == 3)
		{
			for( new x = 1; x <= GetMaxClients(); x++)
			{
				if(!IsClientInGame(x) || GetClientTeam(x) == GetClientTeam(target) || x == target || IsFakeClient(x))
					continue;
				if(bandw_type == 1) PrintHintText(x, "%T","Player is black and white.",x, targetName, charName);
				else PrintToChat(x, "%T","Player is black and white.",x, targetName, charName);
			}
		}
		//print to survivors
		else
