...
//#define PreventSpecBlockInfectedTeamIcon_START_DELAY 2.0
...
public OnPluginStart()
{
	...
	cvarEnforceTeamSwitch = CreateConVar("l4d_teamswitch_enabled", "1", "Can player use command to switch team during game after player leave safe area?", FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_NOTIFY);
...
public Action:eventplayer_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(!EnforceTeamSwitch && IsClientInGame(client) && !IsFakeClient(client))
	{
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon, &subtype, &cmdnum, &tickcount, &seed, mouse[2]) //should prevent players from moving
{
	...
	if(!hasdirectorStart)
		if(IsClientInGame(client) && GetClientTeam(client) == L4D_TEAM_INFECTED && IsPlayerGhost(client))
		{
...
public Action:eventRSLiveCallback(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new Handle:VB = FindConVar("sm_vocalize_guard_enabled");
...
{ 
	...
	if(!readyMode)
	{
		if(!EnforceTeamSwitch && client > 0 && client <=MaxClients && IsClientInGame(client) && !IsFakeClient(client) && GetClientTeam(client) == L4D_TEAM_SURVIVORS)
		{
...
{
	if(!EnforceTeamSwitch && IsClientInGame(client) && !IsFakeClient(client) && GetClientTeam(client) == L4D_TEAM_SURVIVORS && IsPlayerAlive(client))
	{
		decl String:steamID[STEAMID_SIZE];
		GetClientAuthString(client, steamID, STEAMID_SIZE);
		new index = FindStringInArray(arrayclientswitchteam, steamID);
...
public Action:L4D_OnSpawnWitch(const Float:vector[3], const Float:qangle[3])
{
	...
	if(hasleftsaferoom == false || hasdirectorStart == false)
	{
...
{
	if(!EnforceTeamSwitch && client && IsClientInGame(client) && !IsFakeClient(client))
	{
...
public Action:WTF(client, args) //player press m
{
	...
	if(!EnforceTeamSwitch && hasleftsaferoom && GetClientTeam(client) != L4D_TEAM_SPECTATE) 
	{
...
DrawReadyPanelList()
{
	...
	switch(change)
	{
		...
		case 1:
		{
			Format(Notice, 64, "● Slots: %d/%d - %s round", RealplayerinSV(), GetConVarInt(FindConVar("l4d_maxplayers")), (InSecondHalfOfRound)? "2nd": "1st");
		}
		case 2:
		{
	...
	for(i = 1; i <= MaxClients; i++) 
	{
		...
		{
			if(GetClientMenu(i, INVALID_HANDLE) == MenuSource_None) //client is not watching menu
			{
				SendPanelToClient(panel, i, Menu_ReadyPanel, READY_LIST_PANEL_LIFETIME);
				continue;
			}
			else if(IsClientVoteMenu(i)||IsClientInfoMenu(i)){
				continue;
			}
			else
				SendPanelToClient(panel, i, Menu_ReadyPanel, READY_LIST_PANEL_LIFETIME);
...
readyOn()
{
	...
	if(StrEqual(mapbuf, "l4d_vs_city17_01")||StrEqual(mapbuf, "l4d_vs_deadflagblues01_city"))
	{
...
RoundIsLive()
{
	...
	//new i;
	//for(i = 1; i <= MaxClients; i++)
	//	if(IsClientConnected(i) && IsClientInGame(i)&& !IsFakeClient(i) && GetClientTeam(i) == L4D_TEAM_SPECTATE ) 
	//		CreateTimer(PreventSpecBlockInfectedTeamIcon_START_DELAY+i*0.1, Timer_PreventSpecBlockInfectedTeamIcon,i, TIMER_FLAG_NO_MAPCHANGE);
...
public Action:Event_player_left_checkpoint(Handle:event, String:name[], bool:dontBroadcast)
{
	if(hasdirectorStart)
		return Plugin_Handled;
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if(client&&IsClientInGame(client) && IsPlayerAlive(client) && GetClientTeam(client) == L4D_TEAM_SURVIVORS)
	{
		if (readyMode)
		{
...
{
	return GetConVarInt(cvarEnforceReady);
}
...
public OnPreThinkPost(client)
{
	...
	{
		SetEntProp(client, Prop_Send, "m_ghostSpawnState", 2);// unable to spawn: waiting for the survivor to leave the safe room
	}
}
...
{
	if( FloatSub(fTime, g_fButtonTime[client]) > 15.0)
		return true;
	return false;
}
