...
PrintFormattedMessageToAll( String:rawmsg[301], client,playerjoin )//給全部人看的
{
...
PrintFormattedMessageToAdmins( String:rawmsg[301], client , playerjoin)//專屬給adm看的
{
	decl String:message[301];
...
PrintFormattedMsgToNonAdmins( String:rawmsg[301], client, playerjoin )//給不是adm看的
{
	decl String:message[301];
...
GetFormattedMessage( String:rawmsg[301], client, String:outbuffer[], outbuffersize )
{
	...
	if( client > -1 )
	{
		...
		if (StrContains(rawmsg, "{PLAYERNAME}") != -1) 
		{
			...
			ReplaceString(rawmsg, sizeof(rawmsg), "{PLAYERNAME}", player);
		}

		if (StrContains(rawmsg, "{STEAMID}") != -1) 
		{
			GetClientAuthId(client, AuthId_Steam2,STEAMID, sizeof(STEAMID));
			ReplaceString(rawmsg, sizeof(rawmsg), "{STEAMID}", STEAMID);
		}
		
		if (StrContains(rawmsg, "{PLAYERCOUNTRY}") != -1 ) 
		{
			ReplaceString(rawmsg, sizeof(rawmsg), "{PLAYERCOUNTRY}", player_country);
		}
		
		if (StrContains(rawmsg, "{PLAYERCOUNTRYSHORT}") != -1 ) 
		{
			ReplaceString(rawmsg, sizeof(rawmsg), "{PLAYERCOUNTRYSHORT}", player_ccode);
		}
		
		if (StrContains(rawmsg, "{PLAYERCOUNTRYSHORT3}") != -1 ) 
		{
			ReplaceString(rawmsg, sizeof(rawmsg), "{PLAYERCOUNTRYSHORT3}", player_ccode3);
		}
		
		if (StrContains(rawmsg, "{PLAYERCITY}") != -1 ) 
		{
			ReplaceString(rawmsg, sizeof(rawmsg), "{PLAYERCITY}", player_city);
		}
		
		if (StrContains(rawmsg, "{PLAYERREGION}") != -1 ) 
		{
			ReplaceString(rawmsg, sizeof(rawmsg), "{PLAYERREGION}", player_region);
		}
		
		if (StrContains(rawmsg, "{PLAYERIP}") != -1 ) 
		{
			ReplaceString(rawmsg, sizeof(rawmsg), "{PLAYERIP}", player_ip);
		}
