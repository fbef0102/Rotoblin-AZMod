...
OnPostAdminCheck_CountryShow(client)
{
	...
	if( GetConVarInt(g_CvarShowConnect) )
	{
		...
		{
			PrintFormattedMessageToAdmins( rawadmmsg, client,1 );
			PrintFormattedMsgToNonAdmins( rawmsg, client,1 );
		}
		else
		{
			PrintFormattedMessageToAll( rawmsg, client,1 );
		}
	}	
}
...
public Action:event_PlayerDisc_CountryShow(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if( GetConVarInt(g_CvarShowDisconnect) )
	{
		...
		if( KvJumpToKey(hKVCountryShow, "messages_admin", false) )
		{
			...
			{
				ReplaceString(rawadmmsg, sizeof(rawadmmsg), "{DISC_REASON}", dcreason);
		...
		if( KvJumpToKey(hKVCountryShow, "messages", false) )
		{
			...
			{
				ReplaceString(rawmsg, sizeof(rawmsg), "{DISC_REASON}", dcreason);
		...
		if( GetConVarInt(g_CvarShowEnhancedToAdmins) )
		{
			PrintFormattedMessageToAdmins( rawadmmsg, client,0 );
			PrintFormattedMsgToNonAdmins( rawmsg, client,0 );
		}
		else
		{
			PrintFormattedMessageToAll( rawmsg, client,0 );
		}
