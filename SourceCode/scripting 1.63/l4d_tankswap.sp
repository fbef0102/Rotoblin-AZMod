...
public Action:TC_ev_TankSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	DebugPrintToAll("TC_ev_TankSpawn fired, creating Timer");
...
public TS_MenuCallBack(Handle:menu, MenuAction:action, param1, param2)
{
    ...
    if (!choice)
    {
        ...
        {
            L4DD_ReplaceWithBot(choice, true);
        }
        L4DD_ReplaceTank(primaryTankPlayer, choice);
...
stock L4DD_TakeOverZombieBot(client, target)
{
    DebugPrintToAll("TakeOverZombieBot being called, client %N target %N", client, target);
	L4DDirect_TakeOverZombieBot(client,target);
}
...
// do not use with bots. Use L4DD_TakeOverZombieBot instead.
