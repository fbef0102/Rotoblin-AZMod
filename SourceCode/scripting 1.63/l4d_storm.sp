...
public OnPluginStart()
{
	...
	if( !hGamedata) SetFailState("Failed to load \"%s.txt\" gamedata.", GAMEDATA);
	...
	new offset = GameConfGetOffset(hGamedata, "TrySwing_Offset");
	...
	g_Address += (Address:offset);

	for( new i = 0; i < g_ByteCount; i++ )
	{
		g_ByteSaved[i] = LoadFromAddress(g_Address + (Address:i), NumberType_Int8);
	}
	...
	cvarFogEndDist = CreateConVar("sm_envtools_fogend", "500", "離自身中心多少距離才完全起霧(影響特感能生的距離) 0:disable 500:效果", FCVAR_PLUGIN, true, 0.0, true, 8000.0);
...
public Event_heal_success(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(ClientIsInfected[subject])
	{
		...
		PrintHintText(subject,"你治癒了感染(You Cured The Infection)!!");
	}
}
...
ClientGetInfected(attacker,victim)
{
	...
	PrintHintText(victim,"你被感染(You Are %d INFECTED)!!\n使用醫療物品治癒(Use medical item to cure yourself)",ClientIsInfectedTime[victim]);
...
public Action:TimerInfection(Handle:timer, Handle:dataPack)
{
	...
	{
		PrintHintText(client,"你被感染(You Are %d INFECTED)!!\n使用醫療物品治癒(Use medical item to cure)",ClientIsInfectedTime[client]);
...
public Action:PillsUsed_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(ClientIsInfected[client])
	{
		...
		{
			PrintHintText(client,"你治癒了感染(You Cured The Infection)!!");
			ClientIsInfected[client] = false;
		}
		else
			PrintHintText(client,"你治癒了局部感染(You Cured The Partial Infection)!!");
	}
}
...
public Action:COLD_DOWN(Handle:timer,any:client)
{
	...
	{
		PrintHintText(client,"按'向前鍵'爬行\n(Press FORWARD to crawl while incapped)");
	}
}
...
PatchAddress(bool:patch)
{
	...
	if( !patched && patch )
	{
		patched = true;
		for( new i = 0; i < g_ByteCount; i++ )
			StoreToAddress(g_Address + (Address:i), 0x90, NumberType_Int8);
	}
	else if( patched && !patch )
	{
		patched = false;
		for( new i = 0; i < g_ByteCount; i++ )
			StoreToAddress(g_Address + (Address:i), g_ByteSaved[i], NumberType_Int8);
	}
}