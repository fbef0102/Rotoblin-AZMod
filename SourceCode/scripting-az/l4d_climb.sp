...
ConVar l4d_climb_enable;  
ConVar l4d_climb_team;
ConVar l4d_climb_speed[10]; 
ConVar l4d_climb_infected[10];
ConVar h_GameMode;
...
public void OnPluginStart()
{
	...
	//command hook
	AddCommandListener(OnCommandReturnToSafeRoom, "warp_to_start_area");
...
public Action OnCommandReturnToSafeRoom(int client, const char[] command, int args) {
	if(client == 0 || !IsClientInGame(client) || GetClientTeam(client)!=2 || !IsPlayerAlive(client)) return Plugin_Continue;

	if(l4d_climb_enable.BoolValue)
	{
		Stop(client);
	}
...
{
	ResetPlugin();
	if(Is_Ready_Plugin_On()) l4d_climb_enable.SetInt(2);
	else l4d_climb_enable.SetInt(0);
	ResetAllState();
}

public void L4D_OnRoundIsLive() 
{
	l4d_climb_enable.SetInt(0);
	ResetAllState();
}
...
public Action events_to_interrupt(Event event, const char[] name, bool dontBroadcast) 
{
...
void EventInterrupt(Event event, const char[] name = "userid")
{
	if(l4d_climb_enable.BoolValue == false) return;
	
	int victim = GetClientOfUserId(event.GetInt(name)); 
	Interrupt(victim);
}
...
{
 	int mode = l4d_climb_enable.IntValue;
	...
	if(IsValidClient(client))
	{
		...
		{
			int teammode = l4d_climb_team.IntValue;
			...
			else if (IsInfected(client))
			{
				if(teammode == 1 || teammode == 3)
				{
					...
					int m = l4d_climb_infected[c].IntValue;
					if(m == 1)
					{ return true; }
					else if(m == 2 && !isGhost)
					{ return true; }
...
void Start(int client)
{
	...
	if(GetVectorDistance(hit, vOrigin) < gbodywidth * 2.0)
	{
		...
		{ 
			CPrintToChat(client, "[{olive}TS{default}] Unknown model!"); 
		}
	}
}
...
{	
	...
	speedvalue = l4d_climb_speed[speed].FloatValue * l4d_climb_speed[0].FloatValue;
...
{
	bool hit = false;  
	Handle trace = TR_TraceRayFilterEx(pos1, pos2, MASK_SOLID, RayType_EndPoint, DontHitColoeAndOxygentank, client); 
	if(TR_DidHit(trace))
	{
...
public bool TraceRayDontHitSelfAndClone(int entity, int mask, any data)
{
	if(entity == data) 
	{
		return false; 
...
public Action DisplayHint(int client)
{
...
public Action DelayDisplayHint(Handle timer, int client)
{
...
public Action RemoveInstructorHint(Handle timer, DataPack pack)
{
