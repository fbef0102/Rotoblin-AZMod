...
public void OnPluginStart()
{
	...
	g_hCvarAllow =		CreateConVar(	"l4d_dynamic_light_allow",			"1",			"0=Plugin off, 1=Plugin on.", CVAR_FLAGS );
	g_hCvarBots =		CreateConVar(	"l4d_dynamic_light_bots",			"0",			"0=No, 1=Allow bots to have dynamic lights.", CVAR_FLAGS );
	g_hCvarAlpha =		CreateConVar(	"l4d_dynamic_light_bright",			"255.0",		"Brightness of the light.", CVAR_FLAGS );
	g_hCvarColor =		CreateConVar(	"l4d_dynamic_light_color",			"250 250 200",	"The light color. Three values between 0-255 separated by spaces. RGB Color255 - Red Green Blue.", CVAR_FLAGS );
	g_hCvarDefault =	CreateConVar(	"l4d_dynamic_light_default",		"0",			"0=Off, 1=On. Default state of the light when players join.", CVAR_FLAGS );
	g_hCvarDist =		CreateConVar(	"l4d_dynamic_light_distance",		"1000",			"Distance the light shines before not lighting up.", CVAR_FLAGS );
	g_hCvarFade =		CreateConVar(	"l4d_dynamic_light_fade",			"500",			"0=Teleports light to the end distance. Other value starts fading the light brightness from this distance.", CVAR_FLAGS );
	g_hCvarGuns =		CreateConVar(	"l4d_dynamic_light_guns",			"0",			"0=All weapons and items. 1=Guns only. Which items/weapons does the light work on.", CVAR_FLAGS );
	g_hCvarHide =		CreateConVar(	"l4d_dynamic_light_hide",			"2",			"0=Show the dynamic light to all players. 1=Hide the dynamic light so only other players can see it. 2=Only show to the owner.", CVAR_FLAGS );
	g_hCvarHint =		CreateConVar(	"l4d_dynamic_light_hint",			"2",			"1=Print to chat, 2=Hint box.", CVAR_FLAGS );
	g_hCvarHints =		CreateConVar(	"l4d_dynamic_light_hints",			"3",			"0=Off. 1=Show hint when first using the flashlight, 2=Show status on toggle, 3=Both.", CVAR_FLAGS );
	g_hCvarHull =		CreateConVar(	"l4d_dynamic_light_hull",			"1",			"0=Trace directly to where they are aiming. 1=Trace hull to detect nearby entities.", CVAR_FLAGS );
	g_hCvarModes =		CreateConVar(	"l4d_dynamic_light_modes",			"",				"Turn on the plugin in these game modes, separate by commas (no spaces). (Empty = all).", CVAR_FLAGS );
	g_hCvarModesOff =	CreateConVar(	"l4d_dynamic_light_modes_off",		"",				"Turn off the plugin in these game modes, separate by commas (no spaces). (Empty = none).", CVAR_FLAGS );
	g_hCvarModesTog = 	CreateConVar(	"l4d_dynamic_light_modes_tog",		"0",			"Turn on the plugin in these game modes. 0=All, 1=Coop, 2=Survival, 4=Versus, 8=Scavenge. Add numbers together.", CVAR_FLAGS );
