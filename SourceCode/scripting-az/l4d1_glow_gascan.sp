...
1.0.4 (19-June-2022)
    - Forked Glow Item (White) by Marttt: https://forums.alliedmods.net/showthread.php?t=334222
...
// Plugin Info - define
// ====================================================================================================
#define PLUGIN_NAME                   "[L4D1] Glow Gas Can (White)"
#define PLUGIN_AUTHOR                 "Mart, HarryPotter"
#define PLUGIN_DESCRIPTION            "Add a white outline glow effect to gas cans on the map"
...
// Filenames
// ====================================================================================================
#define CONFIG_FILENAME               "l4d1_glow_gascan"
#define DATA_FILENAME                 "l4d1_glow_gascan"
...
public void OnPluginStart()
{
    ...
    CreateConVar("l4d1_glow_gascan_version", PLUGIN_VERSION, PLUGIN_DESCRIPTION, CVAR_FLAGS_PLUGIN_VERSION);
    g_hCvar_Enabled       = CreateConVar("l4d1_glow_gascan_enable", "1", "Enable/Disable the plugin.\n0 = Disable, 1 = Enable.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_Interval      = CreateConVar("l4d1_glow_gascan_interval", "0.3", "Interval in seconds to toggle (start/stop) the glow.", CVAR_FLAGS, true, 0.1);
    ...
    // Admin Commands
    RegAdminCmd("sm_gasglowinfo", CmdInfo, ADMFLAG_ROOT, "Outputs to the chat the glow info about the entity at your crosshair.");
    RegAdminCmd("sm_gasglowreload", CmdReload, ADMFLAG_ROOT, "Reload the glow configs.");
    RegAdminCmd("sm_gasglowremove", CmdRemove, ADMFLAG_ROOT, "Remove plugin glow from entity at crosshair.");
    RegAdminCmd("sm_gasglowremoveall", CmdRemoveAll, ADMFLAG_ROOT, "Remove all glows created by the plugin.");
    RegAdminCmd("sm_gasglowadd", CmdAdd, ADMFLAG_ROOT, "Add glow to entity at crosshair.");
    RegAdminCmd("sm_gasglowall", CmdAll, ADMFLAG_ROOT, "Add glow to everything possible.");
    RegAdminCmd("sm_print_cvars_l4d1_glow_gascan", CmdPrintCvars, ADMFLAG_ROOT, "Print the plugin related cvars and their respective values to the console.");
...
void OnNextFrame(int entityRef)
{
    ...
    //PrintToChatAll("%d %s", entity, modelname);

    if (HasEntProp(entity, Prop_Send, "m_isCarryable")) // CPhysicsProp
    {
        if(g_smPropModelToClassname.GetString(modelname, classname, sizeof(classname)))
        {
            if ( strcmp(classname, "weapon_gascan") == 0)
            {
                if ( g_iDefaultConfig[CONFIG_ENABLE] == 0 )
                    return;
...
int CreatePropGlow(int parent, int[] config)
{
    ...
    DispatchKeyValue(entity, "targetname", "l4d1_glow_gascan");
...
void RemoveAll()
{
    ...
    while ((entity = FindEntityByClassname(entity, "prop_glowing_object")) != INVALID_ENT_REFERENCE)
    {
        ...
        if (StrEqual(targetname, "l4d1_glow_gascan"))
...
Action CmdPrintCvars(int client, int args)
{
    ...
    PrintToConsole(client, "======================================================================");
    PrintToConsole(client, "");
    PrintToConsole(client, "------------------- Plugin Cvars (l4d1_glow_gascan) --------------------");
    PrintToConsole(client, "");
    PrintToConsole(client, "l4d1_glow_gascan_version : %s", PLUGIN_VERSION);
    PrintToConsole(client, "l4d1_glow_gascan_enable : %b (%s)", g_bCvar_Enabled, g_bCvar_Enabled ? "true" : "false");
    PrintToConsole(client, "l4d1_glow_gascan_interval : %.1f", g_fCvar_Interval);
    PrintToConsole(client, "");
