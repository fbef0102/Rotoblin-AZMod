...
public Plugin:myinfo = 
{
    ...
    author = "ProdigySim, Don, Visor, L4D1 port by Harry",
...
public OnPluginStart()
{
    ...
    cvar_allow_tank_ff=CreateConVar("l4d2_infected_ff_allow_tank", "1", "Do not disable friendly fire for tanks on other SI");
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3])
{
    if(GetConVarBool(cvar_ffblock) && IsClientAndInGame(victim) && IsClientAndInGame(attacker) && GetClientTeam(attacker) == 3)
    {
        ...
        if (!GetConVarBool(cvar_allow_tank_ff) || GetEntProp(attacker, Prop_Send, "m_zombieClass") != 5)
        {
