...
#define LEFT4FRAMEWORK_GAMEDATA "left4dhooks.l4d1" // left4dhooks
#define SECTION_NAME "CTerrorPlayer::GetRunTopSpeed"

enum /*L4D1_Infected*/
{
	L4D1Infected_Common = 0,
	L4D1Infected_Smoker = 1,
	L4D1Infected_Boomer,
	L4D1Infected_Hunter,
	L4D1Infected_Witch,
	L4D1Infected_Tank,
	L4D1Infected_Survivor,

	L4D1Infected_Size //7 size
};

stock const char L4D1_InfectedNames[L4D1Infected_Size][] =
{
	"Normal",
	"Smoker",
	"Boomer",
...
Action Cmd_CheckBhop(int iClient, int iArgs)
{
	...
	for (int i = view_as<int>(L4D1Infected_Smoker); i <= view_as<int>(L4D1Infected_Tank); i++) {
		if (i == view_as<int>(L4D1Infected_Witch)) {
			continue;
		}

		ReplyToCommand(iClient, "[Infected] Bunnyhop is %s for %s zombie class!", (IsAllowBhopZClass(i)) ? "allowed" : "blocked", L4D1_InfectedNames[i]);
	}
...
{
	if (iZClass == view_as<int>(L4D1Infected_Tank)) {
		iZClass--;
	}
	iZClass--;

	if (iZClass >= view_as<int>(L4D1Infected_Common)
		&& iZClass <= view_as<int>(L4D1Infected_Hunter)
